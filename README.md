...
## 2. Create a Pipeline
  ...
  you will be prompt for settings :
   ...
   + a pipeline name (one should be set by default)
...
## 3. Create your Staging App 
  ...
  `$ heroku create MY-APP-STAGING --remote staging` note that you can also add some options like the first one we did in Step 1.
  
  We are setting another remote than the base `heroku` one so as to be able to deploy from CLI. 
...
## 4. Add the staging App to your pipeline
 ...
  *note that `PIPELINE_NAME` should be the one set on Step 2 if you didn't chose it is probably the same name as you production App*
...
I hope this could help some of you setting a Staging Env to test your Apps in an actual production-like environment before releasing.
